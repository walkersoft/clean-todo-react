...
function processTodoItemsAll(response: Response): Promise<Types.TodoItemResponse[]> {
 ...
 * @return Success
 */
export function todoItems(body?: Types.CreateTodoItemRequest | undefined): Promise<Types.TodoItemResponse> {
    let url_ = getBaseUrl() + "/api/TodoItems";
      url_ = url_.replace(/[?&]$/, "");
...
function processTodoTagsAll(response: Response): Promise<Types.TodoTagResponse[]> {
    const status = response.status;
    let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
    if (status === 200) {
        return response.text().then((_responseText) => {
 ...
 * @return Success
 */
export function todoTags(body?: Types.CreateTodoTagRequest | undefined): Promise<Types.TodoItemResponse> {
    let url_ = getBaseUrl() + "/api/TodoTags";
      url_ = url_.replace(/[?&]$/, "");
...
function processGetWeatherForecast(response: Response): Promise<Types.WeatherForecast[]> {
    const status = response.status;
    let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
    if (status === 200) {
        return response.text().then((_responseText) => {
